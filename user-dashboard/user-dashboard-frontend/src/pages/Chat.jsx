  ...
  const handleSendMessage = async () => {
    ...
    if (isPlaybookMode) {
      ...
      try {
        ...
        const certPlaybook = certRes.success ? certRes.data.reply : "❌ Failed to generate CERT playbook.";
        ...
        const userPlaybook = userRes.success ? userRes.data.reply : "❌ Failed to generate User playbook.";
    ...
    if (isRiskAnalysisMode) {
      ...
      try {
        ...
        if (!responseData.success) {
          pushAiMessage(`❌ Error: ${responseData.message || "Failed to analyze report"}`);
    ...
    try {
      ...
      if (responseData.success) {
        ...
        pushAiMessage(`❌ ${responseData.message || "Something went wrong."}`);
      }
    } catch (err) {
